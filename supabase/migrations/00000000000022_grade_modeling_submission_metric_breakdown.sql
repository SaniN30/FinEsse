-- Phase 8: add a per-metric correctness breakdown to grade_modeling_submission's
-- return value, so the frontend can render a richer per-metric pass/fail
-- result (vs. quiz grading's single score) without ever exposing the
-- rubric's hidden expected/tolerance values -- only a boolean per metric key.
